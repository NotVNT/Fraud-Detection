...
class NewsService {
  ...
  Future<List<NewsItem>> fetchZNewsArticles({
    int page = 1,
    String category = '',
    int limit = 10,
  }) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Element> articleElements = document.querySelectorAll(
        ...
        // Limit processing to save time and memory
        int maxToProcess = articleElements.length > limit
            ? limit
          ...
          try {
            ...
            if (imageUrl.isEmpty) {
              ...
              if (title != 'No title' && title.isNotEmpty) {
                print(
                  'No image for: ${title.length > 30 ? title.substring(0, 30) : title}...',
                );
              }
            }
            ...
            try {
              if (timeAgo.contains(':')) {
                ...
                if (dateParts.length == 3) {
                  ...
                  int year =
                      int.tryParse(dateParts[2].trim()) ?? DateTime.now().year;
              ...
              } else if (timeAgo.toLowerCase().contains('giờ') ||
                  timeAgo.toLowerCase().contains('tiếng')) {
                // "X hours ago" format
                ...
                if (match != null) {
                  ...
                  timeAgo.toLowerCase().contains('hôm')) {
                // "X days ago" format
              ...
              // Debug info
              print(
                'Article: ${title.length > 40 ? title.substring(0, 40) : title}... | Date: ${publishDate.toString()} | Raw: $timeAgo',
              );
            } catch (e) {
            ...
            final tagElements = article.querySelectorAll(
              '.article-meta .tags a',
            );
        ...
        print(
          'Loaded ${newsItems.length} articles from page $page (limited to $limit)',
        );
      ...
      if (response.statusCode == 200) {
        ...
        if (contentElement != null) {
          ...
          for (var p in paragraphs) {
            contentBlocks.add({'type': 'paragraph', 'content': p.text.trim()});
          }
        ...
        final timeElement = document.querySelector(
          '.the-article-meta .the-article-publish',
        );
